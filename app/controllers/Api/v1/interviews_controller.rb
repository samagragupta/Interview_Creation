...
class Api::V1::InterviewsController < ApplicationController
  ...
  def destroy
    @interview_participants = InterviewParticipant.where(interview_id: params[:interview][:interviewid]).destroy_all
  ...
  def update
    ...
    @start_time =  params[:interview][:start_time]
    @end_time =  params[:interview][:end_time]
    participants = params[:interview][:participants].split(",")
    participant_interviews = []
    participant_interview_times = []
    participants.each do |participant|
      participant_interviews+=(InterviewParticipant.where(participant_id: participant))
